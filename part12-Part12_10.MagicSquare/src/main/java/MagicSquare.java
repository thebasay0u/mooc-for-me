...
public class MagicSquare {
    ...
    private int sumSecondaryDiagonal() {
        int sum = 0;
        for (int i = 0; i < square.length; i++) {
            sum += this.square[i][square.length - i - 1];
        }
        return sum;
    ...
    public ArrayList<Integer> sumsOfDiagonals() {
        int majorDiag = sumMajorDiagonal();
        int secondaryDiag = sumSecondaryDiagonal();

        ArrayList<Integer> diagonals = new ArrayList<>();
        diagonals.add(majorDiag);
        diagonals.add(secondaryDiag);
